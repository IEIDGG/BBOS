...
class CostcoParser:
    ...
    def extract_order_number(
        soup: BeautifulSoup, email_type: str, subject: str = None
    ) -> Optional[str]:
        if subject:
            order_num = CostcoParser.extract_order_number_from_subject(
                subject, email_type
            )
            if order_num:
                return order_num
        ...
        if email_type == "confirmation":
            ...
            if order_link:
                ...
                if CostcoParser._is_valid_costco_order_number(order_num):
                    logger.debug(
                        f"Extracted cancellation order number from HTML: {order_num}"
                    )
                    return order_num
            ...
            if order_link:
                ...
                if CostcoParser._is_valid_costco_order_number(order_num):
                    logger.debug(
                        f"Extracted shipped order number from HTML: {order_num}"
                    )
                    return order_num

            tracking_links = soup.find_all(
                "a", href=lambda h: h and "shipmenttracking.costco.com" in h.lower()
            )
            for link in tracking_links:
                ...
                if match:
                    ...
                    if CostcoParser._is_valid_costco_order_number(order_num):
                        logger.debug(
                            f"Extracted shipped order number from tracking URL: {order_num}"
                        )
                        return order_num
    ...
    def extract_order_date(soup: BeautifulSoup) -> Optional[str]:
                ...
                if parent_tr:
                    ...
                    for sibling in sibling_tds:
                        ...
                        if text and "Order Placed" not in text:
                            ...
                            if date_match:
                                logger.debug(
                                    f"Extracted order date: {date_match.group()}"
                                )
        ...
        for td in all_tds:
            ...
            if "Order Placed" in text and "Order Number" not in text:
                ...
                if date_match:
                    logger.debug(
                        f"Extracted order date from text: {date_match.group()}"
                    )
                    return date_match.group()

        page_text = soup.get_text()
        date_match = re.search(
            r"Order Placed\s*[:\s]*\s*(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2},\s+\d{4}",
            page_text,
        )
        if date_match:
            ...
            if date_part:
                logger.debug(
                    f"Extracted order date from page text: {date_part.group()}"
                )
    ...
    def extract_shipping_address(soup: BeautifulSoup) -> Dict[str, str]:
        ...
        for h3 in soup.find_all("h3"):
            ...
            address_parts = [
                CostcoParser._clean_text(line) for line in raw_text.split("\n")
            ]
    ...
    @staticmethod
    def _populate_address_from_parts(
        address: Dict[str, str], address_parts: List[str]
    ) -> None:
    ...
    def extract_tracking_numbers(
        soup: BeautifulSoup, html_content: str = None
    ) -> List[str]:
        tracking_numbers = []
        ...
        for p in soup.find_all("p"):
            ...
            if match:
                ...
                if (
                    CostcoParser._is_valid_tracking(text_clean)
                    and text_clean not in tracking_numbers
                ):
                        ...
                        if link:
                            ...
                            if text and CostcoParser._is_valid_tracking(text):
                                ...
                                if text_clean not in tracking_numbers:
                                    tracking_numbers.append(text_clean)
                                    logger.info(
                                        f"Extracted tracking from carrier label: {text_clean}"
                                    )

        if not tracking_numbers:
            tracking_links = soup.find_all(
                "a", href=lambda h: h and "emailtracking" in h.lower()
            )
            for link in tracking_links:
                text = CostcoParser._clean_text(link.get_text())
                if text and CostcoParser._is_valid_tracking(text):
                    ...
                    if text_clean not in tracking_numbers:
                        tracking_numbers.append(text_clean)
                        logger.info(
                            f"Extracted tracking from emailtracking link: {text_clean}"
                        )
                ...
                if link:
                    ...
                    if text and CostcoParser._is_valid_tracking(text):
                        ...
                        if text_clean not in tracking_numbers:
                            tracking_numbers.append(text_clean)
                            logger.info(
                                f"Extracted tracking from align-column: {text_clean}"
                            )
            ...
            for link in all_links:
                ...
                if text and CostcoParser._is_valid_tracking(text):
                    ...
                    if text_clean not in tracking_numbers:
                        tracking_numbers.append(text_clean)
                        logger.info(
                            f"Extracted tracking from target=_blank link: {text_clean}"
                        )

        if not tracking_numbers and html_content:
            tracking_numbers = CostcoParser._extract_tracking_regex_fallback(
                html_content
            )
    ...
    def _extract_tracking_regex_fallback(html_content: str) -> List[str]:
        ...
        for match in ups_matches:
            match_upper = match.upper()
            if (
                CostcoParser._is_valid_tracking(match_upper)
                and match_upper not in tracking_numbers
            ):
                tracking_numbers.append(match_upper)
                logger.info(f"Regex fallback - UPS tracking: {match_upper}")

        fedex_patterns = [r"\b\d{12}\b", r"\b\d{15}\b", r"\b\d{20}\b", r"\b\d{22}\b"]
        for pattern in fedex_patterns:
            fedex_matches = re.findall(pattern, html_content)
            for match in fedex_matches:
                if (
                    CostcoParser._is_valid_tracking(match)
                    and match not in tracking_numbers
                ):
    ...
    def parse_product_details(html_content: str) -> Tuple[List[Dict[str, str]], str]:
        ...
        if not item_sections:
            item_sections = soup.find_all(
                "td", style=lambda s: s and "display: inline-block" in s
            )

        for section in item_sections:
            product = {}

            title_td = section.find(
                "td",
                style=lambda s: (
                    s and "font-size: 14px" in s and "line-height: 24px" in s
                ),
            )
            if title_td:
            ...
            for td in tds:
                ...
                if "Item #" in text or "Item#" in text:
                    ...
                    if item_num:
                        product["item_number"] = CostcoParser._clean_text(
                            item_num.group(1)
                        )
                ...
                if "$" in text and "price" not in product:
                    ...
                    if price_match:
                        product["price"] = CostcoParser._normalize_price(
                            price_match.group()
                        )
            ...
            if product.get("title"):
                ...
                if "price" not in product:
                    product["price"] = "N/A"
                item_image = CostcoParser._extract_product_image(
                    section, product.get("title", "")
                )
                if item_image:
                    product["item_image"] = item_image
                    logger.info(
                        "Scraped Costco product image for: %s", product["title"][:80]
                    )
    ...
    def _extract_total_price(soup: BeautifulSoup) -> str:
        ...
        for h2 in h2_tags:
            ...
            if h2_text == "Total" or (
                h2_text.startswith("Total") and "Subtotal" not in h2_text
            ):
                ...
                if parent_tr:
                    ...
                    if price_td:
                        ...
                        if (
                            "$" in price_text
                            or price_text.replace(".", "").replace(",", "").isdigit()
                        ):
    ...
    def extract_price_summary(soup: BeautifulSoup) -> Dict[str, str]:
        summary = {"subtotal": "N/A", "shipping": "N/A", "tax": "N/A", "total": "N/A"}
