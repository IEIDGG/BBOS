...
class AmazonParser:
    ...
    def extract_order_number(
        cls, soup: BeautifulSoup, subject: str = None
    ) -> Optional[str]:
        if subject:
        ...
        if order_marker != -1:
            ...
            if match:
                logger.debug(
                    f"Extracted order number from HTML Order #: {match.group()}"
                )
                return match.group()
        ...
        for a in soup.find_all("a", href=True):
            ...
            if any(pattern in href for pattern in track_url_patterns):
                if "amazon.com" in href or "amzn" in href:
                    logger.debug(
                        f"Extracted track package link from URL pattern: {a['href']}"
                    )
                    return a["href"]
    ...
    @classmethod
    def extract_all_track_package_links(
        ...
        for a in soup.find_all("a", href=True):
            ...
            if not is_track_link:
                ...
                is_track_link = any(
                    pattern in href_lower for pattern in track_url_patterns
                ) and ("amazon.com" in href_lower or "amzn" in href_lower)
            ...
            if is_track_link:
                ...
                if shipment_match and shipment_match not in seen_shipment_ids:
                    ...
                    tracking_data.append(
                        {"tracking_number": shipment_match, "tracking_url": href}
                    )
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if tracking_id not in tracking_numbers:
                    ...
                    logger.debug(
                        f"Extracted tracking/shipment ID from URL: {tracking_id}"
                    )
    ...
    def _extract_asin_from_url(cls, href: str) -> Optional[str]:
        ...
        if decoded_match:
            ...
            asin_match = re.search(
                r"(?:/dp/|/gp/product/|/gp/aw/d/)([A-Z0-9]{10})", decoded_url
            )
            if asin_match:
                return asin_match.group(1)
            ...
            if ancestor.name in ["table", "div", "td"]:
                ...
                for keyword in sponsored_keywords:
                    if keyword in ancestor_text:
                        if (
                            "quantity" not in ancestor_text
                            and "qty" not in ancestor_text
                        ):
                            logger.debug(
                                f"Detected sponsored section keyword: {keyword}"
                            )
                            return True
    ...
    def extract_products(cls, soup: BeautifulSoup) -> List[Dict]:
        ...
        for link in product_links:
                ...
                for pattern in qty_patterns:
                    ...
                    if qty_match:
                        ...
                        if int(qty_val) > 0 and int(qty_val) < 100:
                            ...
                            logger.debug(
                                f"Found quantity {product['quantity']} for {asin}"
                            )
                            break

                price_divs = product_row.find_all(
                    "div", class_=lambda x: x and "rio-text" in str(x)
                )
                if not price_divs:
            ...
            if product["unit_price"]:
                product["price"] = cls._calculate_item_total(
            ...
            logger.debug(
                f"Extracted product: {product['title'][:50]}... qty={product['quantity']}, unit=${product['unit_price']}, total={product['price']}"
            )
                        ...
                        for link in links:
                            ...
                            if "/dp/" in href or "/gp/product/" in href:
                                ...
                                if title and len(title) >= 3:
                                    ...
                                    title_with_asin = (
                                        f"{clean_title} {asin}" if asin else clean_title
                                    )
                                    product = {
                                        "title": title_with_asin,
                                        "asin": asin or "",
                                    ...
                                    for pattern in qty_patterns:
                                        qty_match = re.search(
                                            pattern, row_text, re.IGNORECASE
                                        )
                                        if qty_match:
                                            qty_val = qty_match.group(1)
                                            if int(qty_val) > 0 and int(qty_val) < 100:
                                    ...
                                    if unit_price:
                                        ...
                                        product["price"] = cls._calculate_item_total(
                                            unit_price, product["quantity"]
                                        )

                                    products.append(product)
                                    logger.debug(
                                        f"Extracted product from table: {product['title'][:50]}..."
                                    )
        ...
        for img in soup.find_all("img"):
            ...
            if alt_text and len(alt_text) > 15 and "amazon" not in alt_text.lower():
                ...
                if product["title"] not in [p["title"] for p in products]:
                    products.append(product)
                    logger.debug(
                        f"Extracted product from image alt: {product['title'][:50]}..."
                    )

        if not products:
            quantity_sections = re.findall(
                r"Quantity:\s*\d+[^$]*\$[\d,]+\.?\d*",
                text_content,
            ...
            for section in quantity_sections[:5]:
                ...
                if qty_match and price_match:
                    ...
                    products.append(product)
                    logger.debug(
                        f"Extracted product from text pattern: qty={product['quantity']}, price={product['price']}"
                    )
    ...
    def parse_confirmation_email(cls, html_content: str, subject: str = None) -> Dict:
        ...
        logger.info(
            f"Amazon confirmation - Order: {order_number}, Products: {len(products)}, Total: {grand_total}"
        )
        if not products:
            ...
            product_link_count = sum(
                1
                for a in all_links
                if "/dp/" in a.get("href", "") or "/gp/product/" in a.get("href", "")
            )
    ...
    def parse_shipped_email(cls, html_content: str, subject: str = None) -> Dict:
        ...
        logger.debug(
            f"Shipped email - Order: {order_number}, Products: {len(products)}, Tracking: {tracking_numbers}"
        )
    ...
    def is_shipped_email(cls, from_addr: str, subject: str) -> bool:
        ...
        subject_match = any(
            kw in subject.lower() for kw in ["shipped", "arriving", "package"]
        )
        return from_match or subject_match
