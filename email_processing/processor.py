...
def _fallback_bestbuy_fulfillment(html_content: str, soup: BeautifulSoup, order_parser: OrderParser) -> Dict[str, str]:
    ...
    text = _normalize_text(soup.get_text(" ") if soup is not None else BeautifulSoup(html_content or "", "html.parser").get_text(" "))
    if not details.get("state"):
        details.update(_location_from_text(text))

    delivery_match = _BESTBUY_DELIVERY_RE.search(text)
    if delivery_match:
        details["estimated_delivery"] = _normalize_text(delivery_match.group("delivery"))

    return details


def _extract_bestbuy_fulfillment(html_content: str, soup: BeautifulSoup, order_parser: OrderParser) -> Dict[str, str]:
    try:
        bestbuy_parser = importlib.import_module("services.bestbuy_email_parser")
        extractor = getattr(bestbuy_parser, "extract_bestbuy_confirmation_fulfillment", None)
        ...
        logger.warning("Best Buy fulfilment helper is not available; using local fallback")
    except Exception as exc:
        logger.warning("Best Buy fulfilment helper import failed; using local fallback: %s", exc)
...
class EmailProcessor:
    ...
    def process_confirmation_email(self, email_data: tuple) -> Dict[str, Any]:
        try:
            ...
            fulfillment = _extract_bestbuy_fulfillment(html_content, soup, self.order_parser)
            if fulfillment.get('state'):
                ...
                logger.warning("Best Buy confirmation %s: no zip/state or delivery date found", order_number)
    ...
    def process_cancellation_email(self, email_data: tuple) -> Dict[str, Any]:
        try:
            ...
            cancellation_type = 'payment_declined' if self._is_bestbuy_payment_update(subject, html_content) else 'cancelled'
    ...
    def process_shipped_email(self, email_data: tuple) -> Dict[str, Any]:
        try:
            ...
            if fulfillment.get('zip_and_state'):
                ...
                logger.warning("Best Buy shipped %s: no city/state/zip found", order_number)
    ...
    def process_price_match_credit_email(self, email_data: tuple) -> Dict[str, Any]:
        try:
            ...
            order_number = None
            order_number_label = soup.find(string=re.compile(r'Order number:', re.IGNORECASE))
            if order_number_label:
                parent = order_number_label.parent if order_number_label.parent else None
                if parent:
                    bold = parent.find_next('span', style=re.compile(r'font-weight.*700|font-weight.*bold', re.IGNORECASE))
                    if bold:
            ...
            if not order_number:
                print("Warning: Could not extract order number from price match credit email")
                return {}

            # Extract credit amount (e.g. $100.00)
            amount_saved = None
            credit_match = re.search(r'credit of \$([0-9]+(?:\.[0-9]{2})?)', html_content)
            if credit_match:
                amount_saved = credit_match.group(1)
            if not amount_saved:
                credit_match = re.search(r'\$([0-9]+(?:\.[0-9]{2})?).*(?:credit|price difference)', html_content, re.IGNORECASE)
                if credit_match:
                    amount_saved = credit_match.group(1)

            if not amount_saved:
                print("Warning: Could not extract credit amount from price match credit email")
                return {}

            # Extract product name
            product_name = ''
            # Strategy 1: img alt text (most reliable, always present)
            img_tag = soup.find('img', alt=re.compile(r'Product Image For:', re.IGNORECASE))
            if img_tag:
                alt_text = img_tag.get('alt', '')
                product_name = re.sub(r'^Product Image For:\s*', '', alt_text, flags=re.IGNORECASE).strip()

            # Strategy 2 fallback: link text near "Return Product Details" section
            if not product_name:
                section_header = soup.find(string=re.compile(r'Return Product Details', re.IGNORECASE))
                if section_header:
            ...
            # Extract quantity
            quantity = 1
            qty_label = soup.find('td', string=re.compile(r'^\s*Qty:\s*$', re.IGNORECASE))
            if qty_label:
            ...
            print(f"  Price match credit: Order {order_number} - ${amount_saved} - {product_name or 'Unknown'} x{quantity}")
            return {
                'date': email_date,
                'order_number': order_number,
    ...
    def process_walmart_confirmation_email(self, email_data: tuple) -> Dict[str, Any]:
        ...
        try:
            ...
            order_number = self.walmart_parser.extract_order_number(soup, 'confirmation', subject)
            if not order_number:
                logger.warning("Could not extract Walmart order number")
                return {}

            order_date = self.walmart_parser.extract_order_date(soup)
            products, total_price = self.walmart_parser.parse_product_details(html_content, subject)
    ...
    def process_walmart_cancellation_email(self, email_data: tuple) -> Dict[str, Any]:
        ...
        try:
            ...
            order_number = self.walmart_parser.extract_order_number(soup, 'cancellation', subject)
            if not order_number:
                return {}
    ...
    def process_walmart_shipped_email(self, email_data: tuple) -> Dict[str, Any]:
        ...
        try:
            ...
            order_number = self.walmart_parser.extract_order_number(soup, 'shipped', subject)
            if not order_number:
                return {}

            tracking_numbers = self.walmart_parser.extract_tracking_numbers(soup, html_content)
            shipping_address = self.walmart_parser.extract_shipping_address(soup)
            products, total_price = self.walmart_parser.parse_product_details(html_content, subject)
