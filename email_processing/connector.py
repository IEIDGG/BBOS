...
class EmailConnector:
    ...
    @retry_with_backoff(max_retries=3, base_delay=1)
    def fetch_emails_batch(self, message_ids: List[bytes], use_uid: bool = True) -> Tuple[List[tuple], List[bytes], List[bytes]]:
        results = []
        fetched_ids = []
        ...
        for i in range(0, len(message_ids), self.batch_size):
            ...
            if self.fetch_count >= self.max_fetches_per_session:
                remaining = message_ids[processed_count:]
                print(f"Warning: Reached max fetches per session ({self.max_fetches_per_session})")
                print(f"Processed {processed_count}/{len(message_ids)} emails. {len(remaining)} remaining.")
                return results, fetched_ids, remaining
            ...
            try:
                ...
                result_count = sum(1 for item in msg_data if isinstance(item, tuple) and len(item) >= 2)
                for item in msg_data:
                    if isinstance(item, tuple) and len(item) >= 2:
                        results.append(item)
                
                fetched_ids.extend(batch[:result_count] if result_count <= len(batch) else batch)
    ...
    def renew_session(self) -> bool:
        print(f"\n🔄 Renewing IMAP session (fetch count: {self.fetch_count}/{self.max_fetches_per_session})...")
        try:
            if self.connection:
                try:
                    self.save_progress()
