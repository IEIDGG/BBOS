    ...
    def process_confirmation_emails(self, folder: str) -> List[Dict]:
        ...
        if len(messages) > 10:
            ...
            while remaining_messages:
                email_data_list, fetched_ids, remaining_messages = self.connector.fetch_emails_batch(remaining_messages)
                
                if remaining_messages:
                    session_count += 1
                    print(f"\n🔄 Session {session_count}: Processing {len(remaining_messages)} remaining emails...")
                    if not self.connector.renew_session():
                        print("Failed to renew session. Stopping processing.")
                        break
                    self.connector.connection.select(f'"{folder}"' if ' ' in folder or '/' in folder else folder)
                
                if not email_data_list:
                    break
                
                for fetched_id in fetched_ids:
                    self.connector.mark_uid_processed(fetched_id)
                
                print("⚡ Using parallel processing for email parsing...")
                with ThreadPoolExecutor(max_workers=4) as executor:
                    future_to_data = {executor.submit(self.processor.process_confirmation_email, email_data): email_data 
                                      for email_data in email_data_list if email_data}
                    
                    for future in as_completed(future_to_data):
                        email_data = future_to_data[future]
                        try:
    ...
    def process_cancellation_emails(self, folder: str, orders: List[Dict]) -> None:
        ...
        if len(messages) > 10:
            ...
            while remaining_messages:
                ...
                print("⚡ Using parallel processing for email parsing...")
                with ThreadPoolExecutor(max_workers=4) as executor:
                    future_to_data = {executor.submit(self.processor.process_cancellation_email, email_data): email_data
                                      for email_data in email_data_list if email_data}
    ...
    def process_shipped_emails(self, folder: str, orders: List[Dict], db_manager=None) -> None:
        ...
        if len(messages) > 10:
            ...
            while remaining_messages:
                ...
                print("⚡ Using parallel processing for email parsing...")
                with ThreadPoolExecutor(max_workers=4) as executor:
                    future_to_data = {executor.submit(self.processor.process_shipped_email, email_data): email_data
                                      for email_data in email_data_list if email_data}
    ...
    def process_xbox_emails(self, folder: str) -> List[Dict]:
        ...
        if len(messages) > 10:
            ...
            while remaining_messages:
                ...
                print("⚡ Using parallel processing for email parsing...")
                with ThreadPoolExecutor(max_workers=4) as executor:
                    future_to_data = {executor.submit(self.processor.process_xbox_email, email_data): email_data
                                      for email_data in email_data_list if email_data}
