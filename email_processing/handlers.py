...
from order_extraction.config.settings import SEARCH_CRITERIA, COSTCO_SEARCH_CRITERIA, AMAZON_SEARCH_CRITERIA, WALMART_SEARCH_CRITERIA
...
class OrderEmailHandler(BaseEmailHandler):
    ...
    def _get_cancellation_status(self, result: Dict, mark_payment_declined_as_cancelled: bool) -> str:
        if result.get('cancellation_type') == 'payment_declined':
            return "Cancelled" if mark_payment_declined_as_cancelled else "Payment Declined"
        return "Cancelled"

    def _apply_shipped_location(self, order: Dict, result: Dict, db_manager=None) -> None:
        ...
        location = zip_and_state or (f"{state} {zip_code}".strip() if state or zip_code else '')
        if location:
    ...
    def _apply_cancellation_result(self, result: Dict, orders: List[Dict], mark_payment_declined_as_cancelled: bool) -> None:
        order_number = result.get('order_number')
        if not order_number:
            return

        status = self._get_cancellation_status(result, mark_payment_declined_as_cancelled)
        matched = False
        for order in orders:
                ...
                for future in as_completed(future_to_data):
                    ...
                    try:
                        ...
                        if result.get('order_number'):
                            ...
                            print(f"Processed confirmation: Order {result['order_number']}" + (f" ({location})" if location else ""))
    ...
    def process_cancellation_emails(self, folder: str, orders: List[Dict], ignore_cache: bool = False, date_filter: Optional[str] = None, mark_payment_declined_as_cancelled: bool = True) -> None:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for future in as_completed(future_to_data):
                    ...
                    try:
                        result = future.result()
                        if result.get('order_number'):
                            self._apply_cancellation_result(result, orders, mark_payment_declined_as_cancelled)
                    ...
                    try:
                        ...
                        if result.get('order_number'):
                            ...
                            if not matched and result.get('tracking_numbers'):
                                ...
                                self.statistics['shipped'] += 1
                                self.statistics['tracking_numbers'] += len(result['tracking_numbers'])
                                print(f"Processed shipped: Order {result['order_number']} (from shipped email only)")
    ...
    def process_price_match_credit_emails(self, folder: str, ignore_cache: bool = False, date_filter: Optional[str] = None) -> List[Dict]:
        print(f"\nProcessing Best Buy price match credit emails in folder: {folder}")

        use_uid_filter = not ignore_cache
        search_criteria = get_search_criteria_with_date('price_match_credit', date_filter)
        success, messages = self.connector.search_emails(folder, search_criteria, use_uid_filter=use_uid_filter)
        ...
        if len(messages) > 10:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_data = {executor.submit(self.processor.process_price_match_credit_email, email_data): (email_data, idx)
                                  for idx, email_data in enumerate(email_data_list) if email_data}
                ...
                for future in as_completed(future_to_data):
                    ...
                    try:
                        ...
                        if result.get('order_number') and result.get('amount_saved'):
                            ...
                            print(f"Processed price match credit: Order {result['order_number']} - ${result['amount_saved']}")
                ...
                for future in as_completed(future_to_data):
                    ...
                    try:
                        ...
                        if result.get('order_number'):
                            ...
                            print(f"📦 Amazon SHIPPED: Order {result['order_number']} ({matched_count} items)")
...
class WalmartEmailHandler(BaseEmailHandler):
    ...
    def process_confirmation_emails(self, folder: str, ignore_cache: bool = False, date_filter: Optional[str] = None) -> List[Dict]:
        ...
        use_uid_filter = not ignore_cache
        search_criteria = get_search_criteria_with_date('confirmation', date_filter, WALMART_SEARCH_CRITERIA)
        success, messages = self.connector.search_emails(folder, search_criteria, use_uid_filter=use_uid_filter)
        ...
        if len(messages) > 10:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_data = {executor.submit(self.processor.process_walmart_confirmation_email, email_data): (email_data, idx)
                                  for idx, email_data in enumerate(email_data_list) if email_data}
                ...
                for future in as_completed(future_to_data):
                    ...
                    try:
                        ...
                        if result.get('order_number'):
                            ...
                            self.statistics['confirmations'] += 1
                            print(f"Processed Walmart confirmation: Order {result['order_number']}")
    ...
    def process_cancellation_emails(self, folder: str, orders: List[Dict], ignore_cache: bool = False, date_filter: Optional[str] = None) -> None:
        ...
        use_uid_filter = not ignore_cache
        search_criteria = get_search_criteria_with_date('cancellation', date_filter, WALMART_SEARCH_CRITERIA)
        success, messages = self.connector.search_emails(folder, search_criteria, use_uid_filter=use_uid_filter)
        ...
        if len(messages) > 10:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_data = {executor.submit(self.processor.process_walmart_cancellation_email, email_data): (email_data, idx)
                                  for idx, email_data in enumerate(email_data_list) if email_data}
                        ...
                        if result.get('order_number'):
                            for order in orders:
                                if order['number'] == result['order_number']:
                                    order['status'] = "Cancelled"
                                    self.statistics['cancellations'] += 1
                                    print(f"Processed Walmart cancellation: Order {result['order_number']}")
                                    break
    ...
    def process_shipped_emails(self, folder: str, orders: List[Dict], db_manager=None, ignore_cache: bool = False, date_filter: Optional[str] = None) -> None:
        ...
        use_uid_filter = not ignore_cache
        search_criteria = get_search_criteria_with_date('shipped', date_filter, WALMART_SEARCH_CRITERIA)
        success, messages = self.connector.search_emails(folder, search_criteria, use_uid_filter=use_uid_filter)
        ...
        if len(messages) > 10:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_data = {executor.submit(self.processor.process_walmart_shipped_email, email_data): (email_data, idx)
                                  for idx, email_data in enumerate(email_data_list) if email_data}
                        ...
                        if result.get('order_number'):
                            ...
                            for order in orders:
                                if order['number'] == result['order_number']:
                                    ...
                                    existing_tracking = order.get('tracking', [])
                                    new_tracking = result.get('tracking_numbers', [])
                                    order['tracking'] = list(set(existing_tracking + new_tracking))
                                    ...
                                    self.statistics['shipped'] += 1
                                    self.statistics['tracking_numbers'] += len(new_tracking)
                                    print(f"Processed Walmart shipped: Order {result['order_number']}")
                                    matched = True
                                    break
                            ...
                            if not matched and result.get('tracking_numbers'):
                                ...
                                self.statistics['shipped'] += 1
                                self.statistics['tracking_numbers'] += len(result['tracking_numbers'])
                                print(f"Processed Walmart shipped: Order {result['order_number']} (from shipped email only)")
